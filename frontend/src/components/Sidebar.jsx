    ...
    <div className={`w-60 bg-white hidden border-r border-gray-200 md:flex flex-col justify-between items-center max-sm:absolute top-14 bottom-0 transition-all duration-300 ease-in-out`}>
      <div className='my-7 w-full'>
        <div className='w-full flex justify-center items-center'>
            <span className='rounded-full w-20 h-20 flex items-center justify-center text-4xl font-bold text-white cursor-pointer bg-radial-[at_25%_25%] from-white to-[#086747] to-75%'>{user?.username.charAt(0).toUpperCase()}</span>
        </div>
        <h1 className='mt-1 text-[20px] text-center'>{user?.username}</h1>
      ...
      <div onClick={logout} className='w-full border-t border-gray-200 p-4 px-7 flex items-center gap-3 cursor-pointer rounded-md text-red-600 hover:bg-red-100 duration-300'>
